...
if sys.argv < 1:
        ...
        with open(str(sys.argv[2]),'rb') as fp:
                for i in fp.readlines():
                        storage.add(str(i).replace("\n",''))
        def handler(msfhandler):                                                                                                                             
                data=str(sys.argv[1])
                if data.startswith("https"):                                                                                                                         
                        https=data.split(data[7:8])
                        https[0]="https://"
                ...
                try:
                        ...
                        sys.stdout.write("\r"+str(termcolor.colored("[+] subdomain found -> %s\n" % (handler.maker),"yellow",attrs=['reverse'
])))
                        domain.add(handler.maker)
                        sys.stdout.flush()
                except:
                        sys.stdout.write("\r"+termcolor.colored("[*]Processing -> %s" % (handler.maker),"cyan",attrs=['bold']))
                        sys.stdout.flush()                                                                                                   
